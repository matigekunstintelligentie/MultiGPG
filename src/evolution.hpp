  ...
  void gomea_generation_MO(int macro_generation){
      ...
      if(g::log_pop){
          ...
          for(auto ind:population){
              str += to_string(ind->fitness[0]) + "," + to_string(ind->fitness[1]) + "," + to_string(ind->clusterid) + "," + to_string(clusternr[ind->clusterid]) + "," + to_string(clustered_population[ind->clusterid].size()) + ";";
          }
          str.pop_back();
          str += "\n";
          csv_file << str;
