...
def finetune(sympy_model, X, y, learning_rate=1.0, n_steps=100, 
  ...
  for el in sympy.preorder_traversal(sympy_model):
    if isinstance(el, sympy.Float):
      ...
      str_model = str_model.replace(str(f), "floats[{}]".format(len(floats)-1), 1)
      final_str_model = final_str_model.replace(str(f),"{" + str(len(floats)-1) + "}")
  ...
  expr_vars = sorted([int(el.replace("x_","")) for el in set(re.findall(r'\bx_[0-9]+', pt_str))], reverse=True)
...
# =============================================================================
  ...
  result = sympy.sympify(str(final_str_model.format(*[el.detach().item() for el in best_torch_model]))) if best_torch_model else sympy_model
