...
def model_cleanup(sympy_model):
    ...
    sympy_model = sympy_model.subs(s_inf, sympy.Float(1.0)).replace(
        sympy.zoo, sympy.Float(1.0)
    )
    sympy_model = sympy_model.subs(sympy.conjugate, sympy.Float(1.0)).subs(
        sympy.AccumBounds, sympy.Float(1.0)
    )
    sympy_model = sympy_model.subs(sympy.StrictLessThan, sympy.Float(1.0)).subs(
        sympy.StrictGreaterThan, sympy.Float(1.0)
    )
    sympy_model = sympy_model.replace(sympy.oo, sympy.Float(1.0))
    sympy_model = sympy_model.replace(sympy.S.Infinity, sympy.Float(1.0)).replace(
        sympy.S.NegativeInfinity, sympy.Float(1.0)
    )
    sympy_model = sympy_model.replace(sympy.I, sympy.Float(1.0)).replace(
        sympy.nan, sympy.Float(1.0)
    )
    return sympy_model
...
def sympy_to_numpy_fn(sympy_model):
    def wrapper_fn(_sympy_model, X, extra_local_dict={}):
        ...
        variables_symbols = sympy.symbols(
