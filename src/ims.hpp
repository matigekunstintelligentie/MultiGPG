  ...
  void run()
  {
    ...
    while (!stop) {
      ...
      if (expression_found ||
        ...
        if (g::max_non_improve > 0 &&
            generations_without_improvement >= g::max_non_improve) {
          print("Stopping due to non improvements ",
                generations_without_improvement);
        }
        if (best_train_mses.size() > 0 &&
            best_train_mses[best_train_mses.size() - 1] < 1e-9) {
      ...
      //            fitness_per_tree.resize(g::nr_multi_trees);  // Ensure the
      //            outer vector has 4 initialized inner vectors
      //
      //            string str = "";
      //            for (int tree_number = 0; tree_number < g::nr_multi_trees;
      //            tree_number++) {
      //
      //
      ...
      //
      //                    Vec output =
      //                    ind->trees[tree_number]->get_output(g::fit_func->X_train,
      //                    ind->trees); float fitness = (g::fit_func->y_train -
      //                    output).square().mean();
      //                    fitness_per_tree[tree_number].push_back(fitness); //
      //                    Now it is safe to push_back str +=
      //                    to_string(fitness) + ",";
      //                }
      //                str.pop_back();
      ...
      if (g::log) {
        ...
        for (auto ind : g::ea->MO_archive) {
          ...
          if (best_train_mse > train_mse) {
            ...
            best_string = "(" + to_string(ind->add) + "+(" +
                          to_string(ind->mul) + "*" + ind->human_repr(true) +
                          ")" + ")";
          }
          if (add_comma) {
          ...
          string str_ind = "(" + to_string(ind->add) + "+(" +
                           to_string(ind->mul) + "*" + ind->human_repr(true) +
                           ")" + ")";
          MO_archive_string = MO_archive_string + "[" + to_string(train_mse) +
                              "," + to_string(val_mse) + "," +
        ...
        print(" ~ generation: ",
              ...
              best_train_mse,
              " r2 ",
              to_string(1. - best_train_mse / (g::fit_func->y_train -
        ...
        print(" ~ generation: ",
              macro_generations,
              " evals ",
              g::fit_func->evaluations,
              " ",
              best_train_mses.back(),
              " ",
              generations_without_improvement,
              " ",
              to_string(tock(g::start_time)),
    ...
    if (g::log) {
      ...
      str += best_string + "\t";
      // 7 train variance
      str += to_string((g::fit_func->y_train - g::fit_func->y_train.mean())
                         .square()
                         .mean()) +
             "\t";
      // 8 val variance
      str +=
        to_string(
          (g::fit_func->y_val - g::fit_func->y_val.mean()).square().mean()) +
        "\t";
      ...
      str +=
        to_string(
          consecutive_non_improvements[consecutive_non_improvements.size() -
                                       1]) +
        "\t";
      csv_file << str;
